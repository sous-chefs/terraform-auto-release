locals {
  sous_chefs_hostnames = {
    label_validator                  = "${var.app_hostname_prefix.label_validator}.${local.domain_config.base_domain}"
    changelog_reset                  = "${var.app_hostname_prefix.changelog_reset}.${local.domain_config.base_domain}"
    changelog_validator              = "${var.app_hostname_prefix.changelog_validator}.${local.domain_config.base_domain}"
    cookbook_release_creator         = "${var.app_hostname_prefix.cookbook_release_creator}.${local.domain_config.base_domain}"
    cookbook_supermarket_uploader    = "${var.app_hostname_prefix.cookbook_supermarket_uploader}.${local.domain_config.base_domain}"
    deployment_status_slack_notifier = "${var.app_hostname_prefix.deployment_status_slack_notifier}.${local.domain_config.base_domain}"
    cookbook_release_validator       = "${var.app_hostname_prefix.cookbook_release_validator}.${local.domain_config.base_domain}"
  }
}
...
resource "kubernetes_secret" "webhook-github-sous-chef-frontend" {
  ...
  data = {
    hmac_secret_token     = var.github_hmac_secret
    github_token          = var.github_commit_token
